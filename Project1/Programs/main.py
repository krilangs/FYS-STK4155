...
def ex_d(N, n):
    ...
    #for j, lambd in enumerate(lambda_params):
        ...
        #Mse, R2, Var, Bias = fun.k_fold_CV_franke(X, Y, Z, folds=5, dim=n,
                                #hyperparam=lambd, method="Ridge", train=False)
        #Mse_n, R2_n, Var_n, Bias_n = fun.k_fold_CV_franke(X, Y, Zn, folds=5,
                                #dim=n, hyperparam=lambd, method="Ridge", train=False)
...
if __name__ == "__main__":
    """
    NB! Be careful if running several tasks at once. Running several tasks at
    the same time may affect the results you get, for some reason.
    Comment out the desired task to run (both below and above in the functions)
    """
    #ex_a(N=50, n=5)
    #ex_b(N=50)
    #ex_c(N=50)
